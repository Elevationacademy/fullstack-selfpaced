...
-   Since we used `SERIAL` for the `id` column, PostgreSQL auto-generates the id - so we **do not** include it in the `INSERT` statement. Instead, we list only the columns we want to insert into.
-   In PostgreSQL, strings must be wrapped in **single quotes** (`'...'`) - double quotes are reserved for identifiers (like table/column names)
