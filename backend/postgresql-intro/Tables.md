...
You can check out the other types [here](https://www.w3schools.com/sql/sql_datatypes.asp) or more info on table creation in the [official docs](https://www.postgresql.org/docs/current/sql-createtable.html)

  

To run the above command you can use your PostgreSQL VS Code extension's run query shortcut, or run it via the `psql` command-line tool.

  

To see that your table was created, **refresh your database connection** in the VS Code sidebar - you should see all your columns and their types.

  

Now make sure you **comment out/delete this command** - you only need to `CREATE` a table once - then it's persisted in your DB. However, make sure your connection to the database remains active as you will need this in order to insert, query, etc.
